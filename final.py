...
def infor():
    root2 = tk.Tk()
    root2.geometry("400x220+750+570")
    vvod = message.get()
    num = 0
    for item in tickers:
        num += 1
        if vvod == item:
            label = tk.Label(root2, text = f'Тикер акции: {item}',
                             font='Arial 16', bg='bisque', bd='10', relief='sunken')
            label.pack()
            num2 = 0
            for item2 in shares:
                num2 += 1
                if num2 == num:
                    label = tk.Label(root2, text=f'Название акции: {item2}',
                                     font='Arial 13', bd='1', relief='solid')
                    label.pack()
            num3 = 0
            for item3 in costs:
                num3 += 1
...
def quest1():
    root = tk.Tk()
    root.title("Как работает приложение")
    otvet = tk.Label(root, text="""Введите тикер акции в главном окне заглавными буквами, список тикеров вы можете посмотреть по кнопке 'тикеры'""",
                     font="Arial 14")
    otvet.pack()
...
def spravka():
    root1 = Tk()
    root1.title("Тикеры")
    root1.geometry("250x500+1200+300")
    #root1.resizable(0, 0)
    scrollbar = Scrollbar(root1)
    scrollbar.pack(side=RIGHT, fill=Y)
    tickers_listbox = Listbox(root1, yscrollcommand=scrollbar.set, width=40)
    for ticker in tickers:
        tickers_listbox.insert(END, ticker)
    tickers_listbox.pack(side=LEFT, fill=BOTH)
    scrollbar.config(command=tickers_listbox.yview)
    root1.mainloop()
...
label = tk.Label(text="Приложение для получения информации об акциях",
                 font='Arial 15', bg='navajowhite', bd='13.6', relief='sunken')
label.pack(side='top')
...
btnRightDown = tk.Button(window, text="Тикеры",
                         padx='16',
                         pady='10',
                         font='Arial 15',
                         command=spravka)
